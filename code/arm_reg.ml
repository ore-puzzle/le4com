  ...
  let gen_instr = function
        ...
        let (r2, addr, li) =
          match op1, op2 with
            Reg.IntV i1, Reg.IntV i2 -> 
              (reg_of Reg.reserved_reg, I i2, gen_operand (reg_of Reg.reserved_reg) op1)
    ...
    | Reg.Assign (_, i, op) ->
       let store = 
         match op with
           Param i ->
             if param_to_reg i = A1 then
               [Instr (Ldr (reg_of Reg.reserved_reg, mem_access Sp 0));
                Instr (Str (reg_of Reg.reserved_reg, mem_access A1 i))]
             else
               [Instr (Str (param_to_reg i, mem_access A1 i))]
         | Reg reg -> [Instr (Str (reg_of reg, mem_access A1 i))]
         | _ -> (gen_operand (reg_of Reg.reserved_reg) op) @
                [Instr (Str (reg_of Reg.reserved_reg, mem_access A1 i))] in
       store @ [Instr (Ldr (A1, mem_access Sp 0))]         
    | Reg.Read (dest, op, i) ->
