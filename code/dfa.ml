...
let get_property rslt stmt side =
  match Map.get stmt
          (match side with
             Cfg.BEFORE -> rslt.before | Cfg.AFTER -> rslt.after) with
    Some p -> p
...
let solve anlys cfg =
  ...
  let get_prop m s = match Map.get s m with
      None -> anlys.bottom
    | Some x -> x in
          ...
          (match anlys.compare new_exit_prop old_exit_prop with
           ...
           | GT ->
               List.iter  (* wlにsuccs (preds)を追加 *)
                 ...
                 ((match anlys.direction with
                       FORWARD  -> Cfg.succs
                     | BACKWARD -> Cfg.preds) cfg stmt);
               fixed_point wl
                     (Map.assoc stmt new_entry_prop entries)
                     (Map.assoc stmt new_exit_prop exits)
           | _  -> err "transfer not monotone.")
      | _  -> err "lub operation not adequate." in
  let stmts = Cfg.all_stmts cfg in
  let stmts' = match anlys.direction with
      FORWARD -> stmts
    | BACKWARD -> Array.of_list (List.rev (Array.to_list stmts)) in
  (* entries, exitsの初期値(すべての文のプロパティがbottom) *)
  let init_prop =
    Array.fold_left (fun ip stmt ->
        Map.assoc stmt anlys.bottom ip
      ) Map.empty stmts' in
